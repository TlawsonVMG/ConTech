...
SCHEMA_VERSION = "2026.04.10.quote-line-items"
...
def _create_quote_line_items_table(db):
    if db.table_exists("quote_line_items"):
        return
...
def _migrate_quote_pricing_columns(db):
    if not db.table_exists("quotes"):
        return

    _add_column_if_missing(db, "quotes", "tax_rate_pct", "REAL NOT NULL DEFAULT 0")
    _add_column_if_missing(db, "quotes", "tax_total", "REAL NOT NULL DEFAULT 0")
    _add_column_if_missing(db, "quotes", "grand_total", "REAL NOT NULL DEFAULT 0")
    _add_column_if_missing(db, "quotes", "discount_total", "REAL NOT NULL DEFAULT 0")
    _add_column_if_missing(db, "quotes", "profit_amount", "REAL NOT NULL DEFAULT 0")
    _add_column_if_missing(db, "quotes", "line_item_count", "INTEGER NOT NULL DEFAULT 0")


def _backfill_quote_line_items(db):
    if not db.table_exists("quotes") or not db.table_exists("quote_line_items"):
        return

    quotes = db.execute(
        """
        SELECT id, branch_id, option_name, description, amount, estimated_cost, target_margin_pct
        FROM quotes
        ORDER BY id
        """
    ).fetchall()
    for quote in quotes:
        existing_line = db.execute(
            "SELECT id FROM quote_line_items WHERE quote_id = ? LIMIT 1",
            (quote["id"],),
        ).fetchone()
        if existing_line is None:
            profit_amount = round((quote["amount"] or 0) - (quote["estimated_cost"] or 0), 2)
            db.execute(
                """
                ...
                (
                    ...
                    1,
                    None,
                    quote["option_name"] or "Quote line",
                    quote["description"] or "",
                    "lot",
                    1,
                    quote["estimated_cost"] or 0,
                    quote["amount"] or 0,
                    0,
                    0,
                    0,
        ...
        db.execute(
            """
            UPDATE quotes
            SET tax_rate_pct = COALESCE(tax_rate_pct, 0),
                tax_total = COALESCE(tax_total, 0),
                grand_total = CASE
                    WHEN COALESCE(grand_total, 0) = 0 THEN COALESCE(amount, 0) + COALESCE(tax_total, 0)
                    ELSE grand_total
                END,
                discount_total = COALESCE(discount_total, 0),
                profit_amount = CASE
                    WHEN COALESCE(profit_amount, 0) = 0 THEN COALESCE(amount, 0) - COALESCE(estimated_cost, 0)
                    ELSE profit_amount
                END,
                line_item_count = (
                    SELECT COUNT(*)
...
def apply_non_destructive_migrations(db):
    ...
    _migrate_quote_pricing_columns(db)
    _create_quote_line_items_table(db)
    _backfill_quote_line_items(db)
    ...
    record_schema_version(db, version="2026.04.10.quote-line-items", notes="Quote line item builder and customer invite/profile columns applied")
