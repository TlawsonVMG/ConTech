...
5. Open quotes, jobs, purchasing, dispatch, invoices, and feedback inbox.
6. Submit one `/pilot-feedback` test item from a separate browser session.
7. Confirm the feedback appears in `/feedback/inbox`.
8. Upload and download a small job-document test file if the pilot host has persistent storage mounted.
